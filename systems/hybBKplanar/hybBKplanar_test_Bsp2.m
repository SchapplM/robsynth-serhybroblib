...
RS = hybroblib_create_robot_class('hybBKplanar', '', 'hybBKplanarBsp2');
...
for i = 1:n1
  for j = 1:n2
    for k = 1:n3
         q = [Q1_range(i); Q2_range(j) ; Q3_range(k)];
         Tc_ges = RS.fkine(q);
            if any(~isreal(squeeze(Tc_ges(:))))
              % Wenn es keine Lösung gibt, wird das Ergebnis komplex
              dg_tmp = 0;
            else
                dg_tmp = 1; % Marker für "Teil des Definitionsgebiets"
            end
...
I_iO = Q_data(:,3) == 2;        %in Ordnung
I_tiO = Q_data (:,3) == 1;      %teils in Ordnung
I_niO = Q_data(:,3) == 0;       %nicht in Ordnung
...
legend({'Lösung', 'teilweise Lösung', 'keine Lösung', 'Traj'});
grid on;
