...
# 
# hybBkspatial -> Fünfgelenkkette, Modellierung der Zwangsbedingungen mit trigonometrischer Elimination der Winkel
# kinematic_constraint -> Berechnungen zu kinematischen Zwangsbedingungen

# qJ2 durch qJ3 ersetzt und qJ1 durch qJ2 ersetzt. durch Brünger 12.04.2019
...
r_p1:=<AE*cos(qJ2s), AE*sin(qJ2s)>:
r_p2:=<AB+BC*cos(qJ3s), BC*sin(qJ3s)>:
r1:=ED:
r2:=CD:
...
radx1:=AB+BC*cos(qJ3s)+CD*(cos(qJ3s)*cos(eta_s)-sin(qJ3s)*sin(eta_s)):
rady1:=BC*sin(qJ3s)+CD*(sin(qJ3s)*cos(eta_s)+cos(qJ3s)*sin(eta_s)):
...
racx1:=AE*cos(qJ2s):
racy1:=AE*sin(qJ2s):
racx2:=AB+BC*cos(qJ3s)+CD*(cos(qJ3s)*cos(eta_s)-sin(qJ3s)*sin(eta_s))+ED*(cos(qJ3s)*(cos(eta_s)*cos(xi_s)-sin(eta_s)*sin(xi_s))-sin(qJ3s)*(cos(xi_s)*sin(eta_s)+cos(eta_s)*sin(xi_s))):
racy2:=BC*sin(qJ3s)+CD*(sin(qJ3s)*cos(eta_s)+cos(qJ3s)*sin(eta_s))+ED*(sin(qJ3s)*(cos(eta_s)*cos(xi_s)-sin(eta_s)*sin(xi_s))+cos(qJ3s)*(cos(xi_s)*sin(eta_s)+cos(eta_s)*sin(xi_s))):
...
# rho bestimmen
cos_psi_phi:=cos(qJ3s)*cos(qJ2s)+sin(qJ3s)*sin(qJ2s): # cos(psi-phi)
;
cos_eta_xi:=cos_eta_s*cos_xi_s-sin_eta_s*sin_xi_s: # cos(eta+xi)
;
sin_psi_phi:=sin(qJ3s)*cos(qJ2s)-cos(qJ3s)*sin(qJ2s): # sin(psi-phi)
;
