...
function jv = hybBKspatial_kinconstr_expl_mdh_num_varpar(qJ, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(6,1),zeros(7,1)}
assert(isreal(qJ) && all(size(qJ) == [6 1]), ...
  'hybBKspatial_kinconstr_expl_mdh_num_varpar: qJ has to be [6x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [7 1]), ...
  'hybBKspatial_kinconstr_expl_mdh_num_varpar: pkin has to be [7x1] (double)');

%% Berechnung
% Kinematikparameter und Struktureigenschaften
[beta_mdh, b_mdh, alpha_mdh, a_mdh, theta_mdh, d_mdh, qoffset_mdh] = hybBKspatial_pkin2mdhparam(pkin);
[~, sigma_mdh] = hybBKspatial_structural_kinematic_parameters();

% Kinematik: Einzel-Gelenktransformationen
T = hybBKspatial_joint_trafo_rotmat_mdh_sym_varpar(qJ, pkin);
