...
function Jg_rot = fourbarprisDE2_jacobig_rot_sym_varpar(qJ, link_index, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(1,1),uint8(0),zeros(3,1)}
assert(isreal(qJ) && all(size(qJ) == [1 1]), ...
  'fourbarprisDE2_jacobig_rot_sym_varpar: qJ has to be [1x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'fourbarprisDE2_jacobig_rot_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [3 1]), ...
  'fourbarprisDE2_jacobig_rot_sym_varpar: pkin has to be [3x1] (double)');
Jg_rot=NaN(3,1);
if link_index == 0
...
elseif link_index == 2
	...
	t32 = (-pkin(2) - t33);
	t31 = (-pkin(2) + t33);
	t1 = [0; 0; (pkin(1) ^ 2 - pkin(2) ^ 2 - qJ(1) ^ 2 + (-2 * qJ(1) - pkin(3)) * pkin(3)) / t33 * (-(pkin(1) + t32) * (pkin(1) + t31) * (pkin(1) - t31) * (pkin(1) - t32)) ^ (-0.1e1 / 0.2e1);];
	Jg_rot = t1;
elseif link_index == 3
	%% Symbolic Calculation
