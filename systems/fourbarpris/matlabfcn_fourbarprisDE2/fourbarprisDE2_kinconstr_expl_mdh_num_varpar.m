...
function jv = fourbarprisDE2_kinconstr_expl_mdh_num_varpar(qJ, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(1,1),zeros(3,1)}
assert(isreal(qJ) && all(size(qJ) == [1 1]), ...
  'fourbarprisDE2_kinconstr_expl_mdh_num_varpar: qJ has to be [1x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [3 1]), ...
  'fourbarprisDE2_kinconstr_expl_mdh_num_varpar: pkin has to be [3x1] (double)');

%% Berechnung
% Kinematikparameter und Struktureigenschaften
[beta_mdh, b_mdh, alpha_mdh, a_mdh, theta_mdh, d_mdh, qoffset_mdh] = fourbarprisDE2_pkin2mdhparam(pkin);
[~, sigma_mdh] = fourbarprisDE2_structural_kinematic_parameters();

% Kinematik: Einzel-Gelenktransformationen
T = fourbarprisDE2_joint_trafo_rotmat_mdh_sym_varpar(qJ, pkin);
