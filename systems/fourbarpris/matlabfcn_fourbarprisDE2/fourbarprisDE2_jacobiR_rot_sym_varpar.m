...
function JR_rot = fourbarprisDE2_jacobiR_rot_sym_varpar(qJ, link_index, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(1,1),uint8(0),zeros(3,1)}
assert(isreal(qJ) && all(size(qJ) == [1 1]), ...
  'fourbarprisDE2_jacobiR_rot_sym_varpar: qJ has to be [1x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'fourbarprisDE2_jacobiR_rot_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [3 1]), ...
  'fourbarprisDE2_jacobiR_rot_sym_varpar: pkin has to be [3x1] (double)');
JR_rot=NaN(9,1);
if link_index == 0
...
elseif link_index == 2
	...
	% EndTime: 2020-05-07 09:45:28
	% DurationCPUTime: 0.04s
	% Computational Cost: add. (152->18), mult. (62->23), div. (16->4), fcn. (8->2), ass. (0->16)
	t42 = qJ(1) + pkin(3);
	t46 = (-pkin(2) + t42);
	t38 = (pkin(1) + t46);
	t47 = (-pkin(2) - t42);
	...
	t49 = (t38 * t39);
	t41 = 1 / (t42 ^ 2);
	t48 = t41 * (pkin(1) ^ 2 - pkin(2) ^ 2 + qJ(1) ^ 2 + (2 * qJ(1) + pkin(3)) * pkin(3));
	t37 = pkin(1) - t46;
	t45 = t37 * t49;
	t36 = pkin(1) - t47;
	t44 = sqrt(-(t36 * t45));
	t43 = 1 / pkin(1);
	t40 = 0.1e1 / t42;
	t34 = (-0.1e1 / t44 * t40 * (-t45 + (t49 + (t38 - t39) * t37) * t36) / 0.4e1 + t44 * t41 / 0.2e1) * t43;
	t1 = [t34; (-t48 / 0.2e1 + 0.1e1) * t43; 0; 0; 0; 0; (-t42 * t40 + t48 / 0.2e1) * t43; t34; 0;];
	JR_rot = t1;
elseif link_index == 3
	%% Symbolic Calculation
