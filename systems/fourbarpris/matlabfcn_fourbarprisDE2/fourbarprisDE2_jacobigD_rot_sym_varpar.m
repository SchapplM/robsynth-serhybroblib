...
function JgD_rot = fourbarprisDE2_jacobigD_rot_sym_varpar(qJ, qJD, link_index, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(1,1),zeros(1,1),uint8(0),zeros(3,1)}
assert(isreal(qJ) && all(size(qJ) == [1 1]), ...
  'fourbarprisDE2_jacobigD_rot_sym_varpar: qJ has to be [1x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [1 1]), ...
  'fourbarprisDE2_jacobigD_rot_sym_varpar: qJD has to be [1x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'fourbarprisDE2_jacobigD_rot_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [3 1]), ...
  'fourbarprisDE2_jacobigD_rot_sym_varpar: pkin has to be [3x1] (double)');
JgD_rot=NaN(3,1);
if link_index == 0
...
elseif link_index == 2
	...
	t52 = pkin(1) - t63;
	t60 = t52 * t61;
	t1 = [0; 0; (-0.2e1 + (-0.1e1 / t57 ^ 2 + (-t61 + (t64 + (t54 - t55) * t53) * t52) / t60 / t57 / 0.2e1) * (pkin(1) ^ 2 - pkin(2) ^ 2 - qJ(1) ^ 2 + (-2 * qJ(1) - pkin(3)) * pkin(3))) * (-t60) ^ (-0.1e1 / 0.2e1) * qJD(1);];
	JgD_rot = t1;
elseif link_index == 3
	%% Symbolic Calculation
