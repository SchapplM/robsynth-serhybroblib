...
function JRD_rot = fourbarprisDE1_jacobiRD_rot_sym_varpar(qJ, qJD, link_index, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(1,1),zeros(1,1),uint8(0),zeros(3,1)}
assert(isreal(qJ) && all(size(qJ) == [1 1]), ...
  'fourbarprisDE1_jacobiRD_rot_sym_varpar: qJ has to be [1x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [1 1]), ...
  'fourbarprisDE1_jacobiRD_rot_sym_varpar: qJD has to be [1x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'fourbarprisDE1_jacobiRD_rot_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [3 1]), ...
  'fourbarprisDE1_jacobiRD_rot_sym_varpar: pkin has to be [3x1] (double)');
JRD_rot=NaN(9,1);
if link_index == 0
...
elseif link_index == 1
	...
	% OptimizationMode: 2
	% StartTime: 2020-05-07 09:10:21
	% EndTime: 2020-05-07 09:10:21
	...
	t172 = sqrt(-t176);
	t169 = (t167 * t168);
	t161 = (-t167 + ((-pkin(1) ^ 2 + pkin(2) ^ 2 - qJ(1) ^ 2 + (-2 * qJ(1) - pkin(3)) * pkin(3)) * t169) + 0.2e1 * (t170 / 0.2e1 + qJ(1) / 0.2e1 + pkin(3) / 0.2e1) * t168) * t180;
