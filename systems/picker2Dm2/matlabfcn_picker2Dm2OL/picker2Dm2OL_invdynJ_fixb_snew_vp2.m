...
function tauJ = picker2Dm2OL_invdynJ_fixb_snew_vp2(qJ, qJD, qJDD, g, ...
  pkin, m, mrSges, Ifges)
%% Coder Information
%#codegen
...
assert(isreal(qJ) && all(size(qJ) == [12 1]), ...
  'picker2Dm2OL_invdynJ_fixb_snew_vp2: qJ has to be [12x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [12 1]), ...
  'picker2Dm2OL_invdynJ_fixb_snew_vp2: qJD has to be [12x1] (double)');
assert(isreal(qJDD) && all(size(qJDD) == [12 1]), ...
  'picker2Dm2OL_invdynJ_fixb_snew_vp2: qJDD has to be [12x1] (double)');
assert(isreal(g) && all(size(g) == [3 1]), ...
  'picker2Dm2OL_invdynJ_fixb_snew_vp2: g has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [8 1]), ...
  'picker2Dm2OL_invdynJ_fixb_snew_vp2: pkin has to be [8x1] (double)');
assert(isreal(m) && all(size(m) == [11 1]), ...
  'picker2Dm2OL_invdynJ_fixb_snew_vp2: m has to be [11x1] (double)'); 
assert(isreal(mrSges) && all(size(mrSges) == [11,3]), ...
  'picker2Dm2OL_invdynJ_fixb_snew_vp2: mrSges has to be [11x3] (double)');
assert(isreal(Ifges) && all(size(Ifges) == [11 6]), ...
  'picker2Dm2OL_invdynJ_fixb_snew_vp2: Ifges has to be [11x6] (double)'); 

%% Symbolic Calculation
% From invdyn_fixb_NewtonEuler_linkframe_tauJ_par2_matlab.m
% OptimizationMode: 2
% StartTime: 2020-05-09 23:19:08
...
t1 = [t375 + Ifges(2,3) * qJDD(1) + (-t358 * (m(9) * t313 - t347 * mrSges(9,1) - t349 * mrSges(9,2)) - t367 * (m(9) * t311 + t349 * mrSges(9,1) - t347 * mrSges(9,2)) + t364 * (m(3) * t314 - t348 * mrSges(3,1) - t350 * mrSges(3,2) - t362 * t270 + t371 * t271 + t363 * t272 - t372 * t273 + t360 * t290 - t369 * t291) + t373 * (m(3) * t312 + t350 * mrSges(3,1) - t348 * mrSges(3,2) - t369 * t290 - t360 * t291 + t380 + t387)) * pkin(1) + mrSges(2,1) * t385 - mrSges(2,2) * t386 + t383; t375; t377; t376; Ifges(6,3) * qJDD(5) + mrSges(6,1) * (t318 * g(1) - t319 * g(2)) - mrSges(6,2) * (-t319 * g(1) - t318 * g(2)); t384; Ifges(8,3) * qJDD(7) + mrSges(8,1) * (-t368 * g(1) - t359 * g(2)) - mrSges(8,2) * (-t359 * g(1) + t368 * g(2)); t383; t382; t381; 0; 0;];
tauJ = t1;
