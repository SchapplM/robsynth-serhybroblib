...
function jv = picker2Dm2OL_kinconstr_expl_mdh_num_varpar(qJ, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(12,1),zeros(8,1)}
assert(isreal(qJ) && all(size(qJ) == [12 1]), ...
  'picker2Dm2OL_kinconstr_expl_mdh_num_varpar: qJ has to be [12x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [8 1]), ...
  'picker2Dm2OL_kinconstr_expl_mdh_num_varpar: pkin has to be [8x1] (double)');

%% Berechnung
% Kinematikparameter und Struktureigenschaften
[beta_mdh, b_mdh, alpha_mdh, a_mdh, theta_mdh, d_mdh, qoffset_mdh] = picker2Dm2OL_pkin2mdhparam(pkin);
[~, sigma_mdh] = picker2Dm2OL_structural_kinematic_parameters();

% Kinematik: Einzel-Gelenktransformationen
T = picker2Dm2OL_joint_trafo_rotmat_mdh_sym_varpar(qJ, pkin);
