...
function PhiD_a = picker2Dm2IC_kinconstr_impl_act_jacobianD_mdh_sym_varpar(qJ, qJD, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(12,1),zeros(12,1),zeros(8,1)}
assert(isreal(qJ) && all(size(qJ) == [12 1]), ...
  'picker2Dm2IC_kinconstr_impl_act_jacobianD_mdh_sym_varpar: qJ has to be [12x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [12 1]), ...
  'picker2Dm2IC_kinconstr_impl_act_jacobianD_mdh_sym_varpar: qJD has to be [12x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [8 1]), ...
  'picker2Dm2IC_kinconstr_impl_act_jacobianD_mdh_sym_varpar: pkin has to be [8x1] (double)');
