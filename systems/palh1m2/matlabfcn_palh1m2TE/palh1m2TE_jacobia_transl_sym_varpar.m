...
function Ja_transl = palh1m2TE_jacobia_transl_sym_varpar(qJ, link_index, r_i_i_C, ...
  pkin)
...
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'palh1m2TE_jacobia_transl_sym_varpar: qJ has to be [4x1] (double)');
assert(isa(r_i_i_C,'double') && isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
	'palh1m2TE_jacobia_transl_sym_varpar: Position vector r_i_i_C has to be [3x1] double');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
	'palh1m2TE_jacobia_transl_sym_varpar: link_index has to be [1x1] uint8');
assert(isreal(pkin) && all(size(pkin) == [22 1]), ...
  'palh1m2TE_jacobia_transl_sym_varpar: pkin has to be [22x1] (double)');
Ja_transl=NaN(3,4);
if link_index == 0
