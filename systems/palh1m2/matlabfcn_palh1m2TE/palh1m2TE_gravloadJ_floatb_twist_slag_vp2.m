...
function taug = palh1m2TE_gravloadJ_floatb_twist_slag_vp2(qJ, g, ...
  pkin, m, mrSges)
%% Coder Information
%#codegen
%$cgargs {zeros(4,1),zeros(3,1),zeros(22,1),zeros(11,1),zeros(11,3)}
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'palh1m2TE_gravloadJ_floatb_twist_slag_vp2: qJ has to be [4x1] (double)');
assert(isreal(g) && all(size(g) == [3 1]), ...
  'palh1m2TE_gravloadJ_floatb_twist_slag_vp2: g has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [22 1]), ...
  'palh1m2TE_gravloadJ_floatb_twist_slag_vp2: pkin has to be [22x1] (double)');
assert(isreal(m) && all(size(m) == [11 1]), ...
  'palh1m2TE_gravloadJ_floatb_twist_slag_vp2: m has to be [11x1] (double)'); 
assert(isreal(mrSges) && all(size(mrSges) == [11,3]), ...
  'palh1m2TE_gravloadJ_floatb_twist_slag_vp2: mrSges has to be [11x3] (double)');

%% Symbolic Calculation
% From gravload_joint_floatb_twist_par2_matlab.m
% OptimizationMode: 2
% StartTime: 2020-05-01 20:14:48
% EndTime: 2020-05-01 20:14:50
...
t1 = [(-t54 * t42 + t51 * t48) * g(2) + (-t51 * t42 - t54 * t48) * g(1), (-g(3) * t5 + t59 * t4) * t47 + (t4 * g(3) + t59 * t5) * t41, (t40 * t6 + t8 * t46) * t47 + (-t40 * t8 + t6 * t46) * t41, (-mrSges(6,1) * t26 - mrSges(6,2) * t66) * t45 - t39 * (mrSges(6,1) * t66 - mrSges(6,2) * t26) + t73 * t33 * (t11 * t36 + t56 * t32)];
taug = t1(:);
