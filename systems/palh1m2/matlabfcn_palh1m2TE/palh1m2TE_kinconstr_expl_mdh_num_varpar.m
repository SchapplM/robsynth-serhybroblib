...
function jv = palh1m2TE_kinconstr_expl_mdh_num_varpar(qJ, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(4,1),zeros(22,1)}
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'palh1m2TE_kinconstr_expl_mdh_num_varpar: qJ has to be [4x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [22 1]), ...
  'palh1m2TE_kinconstr_expl_mdh_num_varpar: pkin has to be [22x1] (double)');

%% Berechnung
% Kinematikparameter und Struktureigenschaften
[beta_mdh, b_mdh, alpha_mdh, a_mdh, theta_mdh, d_mdh, qoffset_mdh] = palh1m2TE_pkin2mdhparam(pkin);
[~, sigma_mdh] = palh1m2TE_structural_kinematic_parameters();

% Kinematik: Einzel-Gelenktransformationen
T = palh1m2TE_joint_trafo_rotmat_mdh_sym_varpar(qJ, pkin);
