...
function taug = palh1m2DE1_gravloadJ_floatb_twist_slag_vp1(qJ, g, ...
  pkin, m, rSges)
%% Coder Information
%#codegen
%$cgargs {zeros(4,1),zeros(3,1),zeros(22,1),zeros(11,1),zeros(11,3)}
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'palh1m2DE1_gravloadJ_floatb_twist_slag_vp1: qJ has to be [4x1] (double)');
assert(isreal(g) && all(size(g) == [3 1]), ...
  'palh1m2DE1_gravloadJ_floatb_twist_slag_vp1: g has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [22 1]), ...
  'palh1m2DE1_gravloadJ_floatb_twist_slag_vp1: pkin has to be [22x1] (double)');
assert(isreal(m) && all(size(m) == [11 1]), ...
  'palh1m2DE1_gravloadJ_floatb_twist_slag_vp1: m has to be [11x1] (double)'); 
assert(isreal(rSges) && all(size(rSges) == [11,3]), ...
  'palh1m2DE1_gravloadJ_floatb_twist_slag_vp1: rSges has to be [11x3] (double)');

%% Symbolic Calculation
% From gravload_joint_floatb_twist_par1_matlab.m
% OptimizationMode: 2
% StartTime: 2020-05-01 20:57:37
% EndTime: 2020-05-01 20:57:42
...
t3 = [-(t74 * g(1) + t94 * g(2)) * t60 + (-t94 * g(1) + t74 * g(2)) * t66 - m(8) * (g(1) * (t66 * rSges(8,3) - t9 * t60) + g(2) * (t60 * rSges(8,3) + t9 * t66)) - ((-t10 * t64 + t58 * t11) * t65 - (-rSges(9,3) * g(2) + pkin(15) * g(1)) * t60 + (rSges(9,3) * g(1) + pkin(15) * g(2)) * t66 + (t58 * t10 + t11 * t64) * t59) * m(9) - m(10) * (g(1) * (t66 * rSges(10,3) - t8 * t60) + g(2) * (t60 * rSges(10,3) + t8 * t66)) - m(11) * ((g(1) * rSges(11,3) + g(2) * t77) * t66 + (g(2) * rSges(11,3) - g(1) * t77) * t60) + (-t38 * (t13 * t67 + t14 * t61 + (t61 * t50 + t67 * t53) * t76) + t37 * (-t13 * t61 + t14 * t67 + (t67 * t50 - t61 * t53) * t76)) * t82, (-t2 * g(3) + t1 * t27) * t65 + (t1 * g(3) + t2 * t27) * t59, (t29 - t90) * cos(t43) / 0.2e1 + (t30 + t96) * sin(t43) / 0.2e1 + (t46 + t91) * cos(t34) / 0.2e1 + (t31 - t85) * sin(t34) / 0.2e1 + (t46 - t91) * cos(t33) / 0.2e1 + (t31 + t85) * sin(t33) / 0.2e1 + (t29 + t90) * cos(t42) / 0.2e1 + (t30 - t96) * sin(t42) / 0.2e1 - (t36 * cos(t39) - sin(t39) * t93 - t95 * t41 - t40 * t32) * g(3), -(t81 * t82 + ((-t15 * t50 + t16 * t53) * t38 - (t15 * t53 + t16 * t50) * t37) * t80) * m(6)];
taug = t3(:);
