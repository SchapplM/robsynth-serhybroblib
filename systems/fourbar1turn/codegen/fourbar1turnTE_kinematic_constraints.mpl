...
# 
# fourbar1TE -> Viergelenkkette, modellierung der Zwangsbedingungen mit Ausdr¨¹cken f¨¹r trigonometrische Elimination
...
qJ_t := Matrix(NQJ, 1, <qJ0(t),qJ1(t)>):
qJ_s := Matrix(NQJ, 1, <qJ0s,qJ1s>):
# Konstante Winkel bereits hineinschreiben ( Keine f¨¹r Viergelenkkette)
for i from 1 to RowDimension(kintmp_s) do
  if diff(kintmp_s(i,1), t) = 0 then
    kintmp_qs(i,1) := kintmp_s(i,1):
  end if:
end do:
# Variablen definieren f¨¹r die Hilfswinkel
# 
...
;
# Zwangsbedingungen f¨¹r eta , zweiter Weg zum Kreisschnittpunkt C
...
;
# Zwangsbedingungen f¨¹r xi , zwei Wege zum Punkt D
r_adx1:=l2*cos(qJ1s):
r_ady1:=l2*sin(qJ1s):
