...
function jv = fourbar1turnTE_kinconstr_expl_mdh_num_varpar(qJ, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(2,1),zeros(5,1)}
assert(isreal(qJ) && all(size(qJ) == [2 1]), ...
  'fourbar1turnTE_kinconstr_expl_mdh_num_varpar: qJ has to be [2x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [5 1]), ...
  'fourbar1turnTE_kinconstr_expl_mdh_num_varpar: pkin has to be [5x1] (double)');

%% Berechnung
% Kinematikparameter und Struktureigenschaften
[beta_mdh, b_mdh, alpha_mdh, a_mdh, theta_mdh, d_mdh, qoffset_mdh] = fourbar1turnTE_pkin2mdhparam(pkin);
[~, sigma_mdh] = fourbar1turnTE_structural_kinematic_parameters();

% Kinematik: Einzel-Gelenktransformationen
T = fourbar1turnTE_joint_trafo_rotmat_mdh_sym_varpar(qJ, pkin);
