...
function U = palh3m2DE1_energypot_floatb_twist_slag_vp1(qJ, r_base, g, ...
  pkin, m, rSges)
%% Coder Information
%#codegen
%$cgargs {zeros(4,1),zeros(3,1),zeros(3,1),zeros(18,1),zeros(9,1),zeros(9,3)}
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'palh3m2DE1_energypot_floatb_twist_slag_vp1: qJ has to be [4x1] (double)');
assert(isreal(r_base) && all(size(r_base) == [3 1]), ...
  'palh3m2DE1_energypot_floatb_twist_slag_vp1: r_base has to be [3x1] (double)');
assert(isreal(g) && all(size(g) == [3 1]), ...
  'palh3m2DE1_energypot_floatb_twist_slag_vp1: g has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [18 1]), ...
  'palh3m2DE1_energypot_floatb_twist_slag_vp1: pkin has to be [18x1] (double)');
assert(isreal(m) && all(size(m) == [9 1]), ...
  'palh3m2DE1_energypot_floatb_twist_slag_vp1: m has to be [9x1] (double)'); 
assert(isreal(rSges) && all(size(rSges) == [9,3]), ...
  'palh3m2DE1_energypot_floatb_twist_slag_vp1: rSges has to be [9x3] (double)');

%% Symbolic Calculation
% From energy_potential_floatb_twist_worldframe_par1_matlab.m
% OptimizationMode: 2
% StartTime: 2020-05-07 01:57:40
% EndTime: 2020-05-07 01:57:41
...
t5 = (-t1 * g(3) + t2 * t49) * cos(t22) + (-t2 * g(3) - t1 * t49) * sin(t22) + (t43 * g(1) + t59 * g(2)) * t39 + (-t59 * g(1) + t43 * g(2)) * t33 + t4 * g(3) * t38 - t3 * g(3) * t32 - m(6) * pkin(11) * g(3) - g(3) * (m(2) * rSges(2,3) + pkin(13) * m(7) + t51 * pkin(11)) - m(8) * (g(1) * (rSges(8,3) * t33 + t39 * t6 + r_base(1)) + g(2) * (-rSges(8,3) * t39 + t33 * t6 + r_base(2)) + g(3) * ((rSges(8,1) * t26 - rSges(8,2) * t28) * t40 + t34 * (rSges(8,1) * t28 + rSges(8,2) * t26) + t52)) - m(9) * ((-g(2) * rSges(9,3) + g(1) * t45) * t39 + (g(1) * rSges(9,3) + g(2) * t45) * t33 + (-t18 * rSges(9,1) - t19 * rSges(9,2) + t52 + (t27 * t46 + t29 * t47) * pkin(3)) * g(3) + t60) + (-rSges(1,1) * g(1) - rSges(1,2) * g(2) - rSges(1,3) * g(3)) * m(1) + (-g(3) * r_base(3) - t60) * (m(6) + m(1) + t51);
U = t5;
