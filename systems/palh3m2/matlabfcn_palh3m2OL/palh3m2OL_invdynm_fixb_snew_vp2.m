...
function m_new = palh3m2OL_invdynm_fixb_snew_vp2(qJ, qJD, qJDD, g, ...
  pkin, m, mrSges, Ifges)
%% Coder Information
%#codegen
%$cgargs {zeros(10,1),zeros(10,1),zeros(10,1),zeros(3,1),zeros(16,1),zeros(9,1),zeros(9,3),zeros(9,6)}
assert(isreal(qJ) && all(size(qJ) == [10 1]), ...
  'palh3m2OL_invdynm_fixb_snew_vp2: qJ has to be [10x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [10 1]), ...
  'palh3m2OL_invdynm_fixb_snew_vp2: qJD has to be [10x1] (double)');
assert(isreal(qJDD) && all(size(qJDD) == [10 1]), ...
  'palh3m2OL_invdynm_fixb_snew_vp2: qJDD has to be [10x1] (double)');
assert(isreal(g) && all(size(g) == [3 1]), ...
  'palh3m2OL_invdynm_fixb_snew_vp2: g has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [16 1]), ...
  'palh3m2OL_invdynm_fixb_snew_vp2: pkin has to be [16x1] (double)');
assert(isreal(m) && all(size(m) == [9 1]), ...
  'palh3m2OL_invdynm_fixb_snew_vp2: m has to be [9x1] (double)'); 
assert(isreal(mrSges) && all(size(mrSges) == [9,3]), ...
  'palh3m2OL_invdynm_fixb_snew_vp2: mrSges has to be [9x3] (double)');
assert(isreal(Ifges) && all(size(Ifges) == [9 6]), ...
  'palh3m2OL_invdynm_fixb_snew_vp2: Ifges has to be [9x6] (double)'); 

%% Symbolic Calculation
% From invdyn_fixb_NewtonEuler_linkframe_m_i_i_par2_matlab.m
% OptimizationMode: 2
% StartTime: 2020-05-07 04:34:13
% EndTime: 2020-05-07 04:35:01
...
t423 = -t432 + t358 * t342 + t359 * t343 + (-m(4) - m(8)) * t337 - t360 * t344 - t361 * t345 - t317 * mrSges(8,2) - t318 * mrSges(4,2) + t315 * mrSges(8,1) + t316 * mrSges(4,1) - t223;
...
t188 = qJDD(1) * mrSges(2,1) + (-t406 * t379 - t410 * t380) * qJD(1) + t422 - t420 * mrSges(2,2) + m(2) * t383 + t433;
...
t171 = Ifges(2,6) * qJDD(1) + mrSges(2,1) * g(3) + t420 * Ifges(2,5) + pkin(6) * (t414 * t295 + t406 * t296) - pkin(12) * (t418 * t179 + t410 * t180) + pkin(13) * t436 + mrSges(2,3) * t384 + (-t434 - t435) * qJD(1) - t428 + t452;
