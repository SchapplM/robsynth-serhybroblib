...
function jv = mg10hlDE1_kinconstr_expl_mdh_num_varpar(qJ, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(6,1),zeros(17,1)}
assert(isreal(qJ) && all(size(qJ) == [6 1]), ...
  'mg10hlDE1_kinconstr_expl_mdh_num_varpar: qJ has to be [6x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [17 1]), ...
  'mg10hlDE1_kinconstr_expl_mdh_num_varpar: pkin has to be [17x1] (double)');

%% Berechnung
% Kinematikparameter und Struktureigenschaften
[beta_mdh, b_mdh, alpha_mdh, a_mdh, theta_mdh, d_mdh, qoffset_mdh] = mg10hlDE1_pkin2mdhparam(pkin);
[~, sigma_mdh] = mg10hlDE1_structural_kinematic_parameters();

% Kinematik: Einzel-Gelenktransformationen
T = mg10hlDE1_joint_trafo_rotmat_mdh_sym_varpar(qJ, pkin);
