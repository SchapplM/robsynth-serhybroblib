...
function jv = mg10hlOL_kinconstr_expl_mdh_num_varpar(qJ, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(13,1),zeros(16,1)}
assert(isreal(qJ) && all(size(qJ) == [13 1]), ...
  'mg10hlOL_kinconstr_expl_mdh_num_varpar: qJ has to be [13x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [16 1]), ...
  'mg10hlOL_kinconstr_expl_mdh_num_varpar: pkin has to be [16x1] (double)');

%% Berechnung
% Kinematikparameter und Struktureigenschaften
[beta_mdh, b_mdh, alpha_mdh, a_mdh, theta_mdh, d_mdh, qoffset_mdh] = mg10hlOL_pkin2mdhparam(pkin);
[~, sigma_mdh] = mg10hlOL_structural_kinematic_parameters();

% Kinematik: Einzel-Gelenktransformationen
T = mg10hlOL_joint_trafo_rotmat_mdh_sym_varpar(qJ, pkin);
