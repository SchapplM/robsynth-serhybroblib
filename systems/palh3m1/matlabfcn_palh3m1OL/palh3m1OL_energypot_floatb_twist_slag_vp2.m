...
function U = palh3m1OL_energypot_floatb_twist_slag_vp2(qJ, r_base, g, ...
  pkin, m, mrSges)
%% Coder Information
%#codegen
%$cgargs {zeros(10,1),zeros(3,1),zeros(3,1),zeros(16,1),zeros(9,1),zeros(9,3)}
assert(isreal(qJ) && all(size(qJ) == [10 1]), ...
  'palh3m1OL_energypot_floatb_twist_slag_vp2: qJ has to be [10x1] (double)');
assert(isreal(r_base) && all(size(r_base) == [3 1]), ...
  'palh3m1OL_energypot_floatb_twist_slag_vp2: r_base has to be [3x1] (double)');
assert(isreal(g) && all(size(g) == [3 1]), ...
  'palh3m1OL_energypot_floatb_twist_slag_vp2: g has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [16 1]), ...
  'palh3m1OL_energypot_floatb_twist_slag_vp2: pkin has to be [16x1] (double)');
assert(isreal(m) && all(size(m) == [9 1]), ...
  'palh3m1OL_energypot_floatb_twist_slag_vp2: m has to be [9x1] (double)'); 
assert(isreal(mrSges) && all(size(mrSges) == [9,3]), ...
  'palh3m1OL_energypot_floatb_twist_slag_vp2: mrSges has to be [9x3] (double)');
...
t26 = sin(qJ(5));
t30 = cos(qJ(5));
t52 = m(6) * pkin(8) + t30 * mrSges(6,1) - t26 * mrSges(6,2) + mrSges(5,1);
t51 = -m(4) - m(8);
t50 = -m(6) - m(5);
t49 = -m(7) - m(3) - m(2);
t47 = -m(1) - m(9) + t49;
