...
function [q, Phi] = palh3m1OL_invkin_eulangresidual(xE_soll, q0, s)

%% Coder Information
%#codegen
%$cgargs {zeros(6,1),zeros(10,1),struct(
...
for rr = 0:retry_limit % Schleife über Neu-Anfänge der Berechnung
  ...
  for jj = 2:n_max % Schleife über iteratives Annähern mit Newton-Verfahren
    ...
    Jdk_voll = palh3m1OL_constr2grad(q1, xE_soll, pkin, T_N_E, phiconv_W_E, I_EElink, reci);
    % Zwangsbedingungen
    Phi_voll = palh3m1OL_constr2(q1, xE_soll, pkin, T_N_E, phiconv_W_E, I_EElink, reci);
