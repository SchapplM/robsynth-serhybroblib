...
function T = palh3m1DE1_energykin_floatb_twist_slag_vp2(qJ, qJD, V_base, ...
  pkin, m, mrSges, Ifges)
%% Coder Information
%#codegen
%$cgargs {zeros(4,1),zeros(4,1),zeros(6,1),zeros(19,1),zeros(9,1),zeros(9,3),zeros(9,6)}
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'palh3m1DE1_energykin_floatb_twist_slag_vp2: qJ has to be [4x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [4 1]), ...
  'palh3m1DE1_energykin_floatb_twist_slag_vp2: qJD has to be [4x1] (double)');
assert(isreal(V_base) && all(size(V_base) == [6 1]), ...
  'palh3m1DE1_energykin_floatb_twist_slag_vp2: V_base has to be [6x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [19 1]), ...
  'palh3m1DE1_energykin_floatb_twist_slag_vp2: pkin has to be [19x1] (double)');
assert(isreal(m) && all(size(m) == [9 1]), ...
  'palh3m1DE1_energykin_floatb_twist_slag_vp2: m has to be [9x1] (double)'); 
assert(isreal(mrSges) && all(size(mrSges) == [9,3]), ...
  'palh3m1DE1_energykin_floatb_twist_slag_vp2: mrSges has to be [9x3] (double)');
assert(isreal(Ifges) && all(size(Ifges) == [9 6]), ...
  'palh3m1DE1_energykin_floatb_twist_slag_vp2: Ifges has to be [9x6] (double)'); 

%% Symbolic Calculation
% From energy_kinetic_floatb_twist_linkframe_par2_matlab.m
% OptimizationMode: 2
% StartTime: 2020-04-18 10:25:32
% EndTime: 2020-04-18 10:26:24
% DurationCPUTime: 43.97s
...
t2 = t161 * t9 + t167 * t4;
t1 = -t161 * t4 + t167 * t9;
t21 = (t111 * mrSges(8,2) - t52 * mrSges(8,3) + Ifges(8,1) * t57 / 0.2e1) * t57 + (V_base(3) * mrSges(2,2) - t126 * mrSges(2,3) + Ifges(2,5) * t153 + Ifges(2,1) * t144 / 0.2e1) * t144 + (-t111 * mrSges(8,1) + t53 * mrSges(8,3) + Ifges(8,4) * t57 + Ifges(8,2) * t56 / 0.2e1) * t56 + (-V_base(3) * mrSges(1,1) + V_base(1) * mrSges(1,3) + Ifges(1,6) * V_base(6) + Ifges(1,2) * V_base(5) / 0.2e1) * V_base(5) + (-t119 * mrSges(3,1) + t116 * mrSges(3,3) + Ifges(3,4) * t123 + Ifges(3,6) * t140 + Ifges(3,2) * t122 / 0.2e1) * t122 + (t126 * mrSges(2,1) - t127 * mrSges(2,2) + Ifges(2,3) * t153 / 0.2e1) * t153 + (t14 * mrSges(5,1) - t15 * mrSges(5,2) + Ifges(5,3) * t7 / 0.2e1) * t7 + (-t111 * mrSges(4,1) + t100 * mrSges(4,3) + Ifges(4,4) * t109 + Ifges(4,6) * t137 + Ifges(4,2) * t108 / 0.2e1) * t108 + (t48 * mrSges(9,2) - t10 * mrSges(9,3) + Ifges(9,5) * t8 + Ifges(9,1) * t13 / 0.2e1) * t13 + (-V_base(3) * mrSges(2,1) + t127 * mrSges(2,3) + Ifges(2,4) * t144 + Ifges(2,6) * t153 + Ifges(2,2) * t143 / 0.2e1) * t143 + (t10 * mrSges(9,1) - t11 * mrSges(9,2) + Ifges(9,3) * t8 / 0.2e1) * t8 + m(2) * (t126 ^ 2 + t127 ^ 2 + t173) / 0.2e1 + m(1) * (V_base(1) ^ 2 + V_base(2) ^ 2 + t173) / 0.2e1 + m(4) * (t100 ^ 2 + t99 ^ 2 + t110) / 0.2e1 + m(8) * (t52 ^ 2 + t53 ^ 2 + t110) / 0.2e1 + m(7) * (t117 ^ 2 + t54 ^ 2 + t55 ^ 2) / 0.2e1 + (V_base(3) * mrSges(1,2) - V_base(2) * mrSges(1,3) + Ifges(1,4) * V_base(5) + Ifges(1,5) * V_base(6) + Ifges(1,1) * V_base(4) / 0.2e1) * V_base(4) + (t111 * mrSges(4,2) - t99 * mrSges(4,3) + Ifges(4,5) * t137 + Ifges(4,1) * t109 / 0.2e1) * t109 + (t3 * mrSges(6,2) - t1 * mrSges(6,3) + Ifges(6,1) * t6 / 0.2e1) * t6 + (t101 * mrSges(5,2) - t14 * mrSges(5,3) + Ifges(5,5) * t7 + Ifges(5,1) * t18 / 0.2e1) * t18 + (-t3 * mrSges(6,1) + t2 * mrSges(6,3) + Ifges(6,4) * t6 + Ifges(6,2) * t5 / 0.2e1) * t5 + m(3) * (t115 ^ 2 + t116 ^ 2 + t119 ^ 2) / 0.2e1 + (-t101 * mrSges(5,1) + t15 * mrSges(5,3) + Ifges(5,4) * t18 + Ifges(5,6) * t7 + Ifges(5,2) * t17 / 0.2e1) * t17 + (t119 * mrSges(3,2) - t115 * mrSges(3,3) + Ifges(3,5) * t140 + Ifges(3,1) * t123 / 0.2e1) * t123 + (-t48 * mrSges(9,1) + t11 * mrSges(9,3) + Ifges(9,4) * t13 + Ifges(9,6) * t8 + Ifges(9,2) * t12 / 0.2e1) * t12 + (t115 * mrSges(3,1) - t116 * mrSges(3,2) + Ifges(3,3) * t140 / 0.2e1) * t140 + (t117 * mrSges(7,2) - t54 * mrSges(7,3) + Ifges(7,1) * t61 / 0.2e1) * t61 + (t52 * mrSges(8,1) - t53 * mrSges(8,2) + Ifges(8,5) * t57 + Ifges(8,6) * t56 + Ifges(8,3) * t39 / 0.2e1) * t39 + (V_base(2) * mrSges(1,1) - V_base(1) * mrSges(1,2) + Ifges(1,3) * V_base(6) / 0.2e1) * V_base(6) + (t99 * mrSges(4,1) - t100 * mrSges(4,2) + Ifges(4,3) * t137 / 0.2e1) * t137 + (t54 * mrSges(7,1) - t55 * mrSges(7,2) + Ifges(7,5) * t61 + Ifges(7,6) * t60 + Ifges(7,3) * t40 / 0.2e1) * t40 + m(5) * (t101 ^ 2 + t14 ^ 2 + t15 ^ 2) / 0.2e1 + (t1 * mrSges(6,1) - t2 * mrSges(6,2) + Ifges(6,5) * t6 + Ifges(6,6) * t5 + Ifges(6,3) * t16 / 0.2e1) * t16 + m(9) * (t10 ^ 2 + t11 ^ 2 + t48 ^ 2) / 0.2e1 + m(6) * (t1 ^ 2 + t2 ^ 2 + t3 ^ 2) / 0.2e1 + (-t117 * mrSges(7,1) + t55 * mrSges(7,3) + Ifges(7,4) * t61 + Ifges(7,2) * t60 / 0.2e1) * t60;
T = t21;
