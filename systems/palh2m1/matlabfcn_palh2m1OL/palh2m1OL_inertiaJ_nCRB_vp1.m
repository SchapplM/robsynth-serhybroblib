...
function H = palh2m1OL_inertiaJ_nCRB_vp1(q, pkin, m_num, rSges_num_mdh, Icges_num_mdh)
%%Coder Information
%#codegen
%$cgargs {zeros(5,1),zeros(6,1),zeros(6,1),zeros(6,3),zeros(6,6)}
assert(isreal(q) && all(size(q) == [5 1]), ...
  'palh2m1OL_inertiaJ_nCRB_vp1: q has to be [5x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [6 1]), ...
  'palh2m1OL_inertiaJ_nCRB_vp1: Kinematic parameters pkin have to be [6x1] (double)');
assert(isreal(m_num) && all(size(m_num) == [6 1]), ...
  'palh2m1OL_inertiaJ_nCRB_vp1: m_num has to be [6x1] (double)');
assert(isreal(rSges_num_mdh) && all(size(rSges_num_mdh) == [6,3]), ...
  'palh2m1OL_inertiaJ_nCRB_vp1: rSges_num_mdh has to be [6x3] (double)');
assert(isreal(Icges_num_mdh) && all(size(Icges_num_mdh) == [6 6]), ...
  'palh2m1OL_inertiaJ_nCRB_vp1: Icges_num_mdh has to be [6x6] (double)');

%% Init
[v_mdh, sigma] = palh2m1OL_structural_kinematic_parameters();
...
%% Kinematik
T_mdh = palh2m1OL_joint_trafo_rotmat_mdh_sym_varpar(q, pkin);
...
I_c_ges = NaN(5,36);
for i = 1:5 % loop through all bodies, [Featherstone2008] T6.2, Line 2
...
for i = 5:-1:1 % Wird die Schleife mit uint8 definiert, funktioniert der generierte mex-Code nicht (Matlab-Bug)
  ...
  for tmp = 1:5 % Dummy-Schleife für Kompilierbarkeit. Abbruchkriterium s.u.
