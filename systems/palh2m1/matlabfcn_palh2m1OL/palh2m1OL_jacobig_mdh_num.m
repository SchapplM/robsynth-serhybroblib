...
function Jg_C = palh2m1OL_jacobig_mdh_num(qJ, link_index, r_i_i_C, pkin)
%% Init
%#codegen
%$cgargs {zeros(5,1),uint8(zeros(1,1)),zeros(3,1),zeros(6,1)}
assert(isreal(qJ) && all(size(qJ) == [5 1]), ...
  'palh2m1OL_jacobig_mdh_num: Joint angles qJ have to be [5x1] (double)');
assert(isa(link_index,'uint8') && all(size(link_index) == [1 1]), ...
  'palh2m1OL_jacobig_mdh_num: link_index has to be [1x1] uint8');
assert(isreal(r_i_i_C) && all(size(r_i_i_C) == [3 1]), ...
  'palh2m1OL_jacobig_mdh_num: Position vector r_i_i_C has to be [3x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [6 1]), ...
  'palh2m1OL_jacobig_mdh_num: Kinematic parameters pkin have to be [6x1] (double)');
...
[v_mdh, sigma_mdh] = palh2m1OL_structural_kinematic_parameters();
