...
%   von `fourbar2OL_invkin_eulangresidual`
%
% Ausgabe:
% Q
...
% Datum: 2019-10-11 13:45
% Revision: 8f8c92630c9cacc8353f8a02babe92eab8225561 (2019-10-11)
% Moritz Schappler, moritz.schappler@imes.uni-hannover.de
% (C) Institut für Mechatronische Systeme, Universität Hannover
% Moritz Schappler, moritz.schappler@imes.uni-hannover.de, 2019-02
% (C) Institut für Mechatronische Systeme, Universität Hannover

function [Q, QD, QDD, PHI] = fourbar2OL_invkin_traj(X, XD, XDD, T, q0, s)

%% Coder Information
%#codegen
%$cgargs {coder.newtype('double',[inf,6]),coder.newtype('double',[inf,6]),
%$cgargs  coder.newtype('double',[inf,6]),coder.newtype('double',[inf,1]),
%$cgargs zeros(4,1), struct(
%$cgargs            'pkin', zeros(2,1),
%$cgargs          'sigmaJ', zeros(4,1),
%$cgargs             'NQJ', 0,
%$cgargs            'qlim', zeros(4,2),
...
for k = 1:nt
  ...
  [q_k, Phi_k] = fourbar2OL_invkin_eulangresidual(X(k,:)', qk0, s);

  %% Gelenk-Geschwindigkeit berechnen (Siehe [1]).
  % Geometrische Jacobi-Matrix in analytische Jacobi umrechnen
  Jg = fourbar2OL_jacobig_sym_varpar(q_k, link_index, r_i_i_C, pkin);
  ...
  JgD = fourbar2OL_jacobigD_sym_varpar(q_k, qD_k, link_index, r_i_i_C, pkin);
