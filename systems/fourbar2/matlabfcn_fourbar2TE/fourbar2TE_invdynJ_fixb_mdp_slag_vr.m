...
%   see fourbar2TE_invdynJ_fixb_regmin2vec.m
% MDP [3x1]
...
function tauJ = fourbar2TE_invdynJ_fixb_mdp_slag_vr(RV, MDP)
%% Coder Information
%#codegen
%$cgargs {zeros(3,1), zeros(3,1)}
assert(isreal(MDP) && all(size(MDP) == [3 1]), ...
  'fourbar2TE_invdynJ_fixb_mdp_slag_vr: MDP has to be [3x1] (double)'); 
