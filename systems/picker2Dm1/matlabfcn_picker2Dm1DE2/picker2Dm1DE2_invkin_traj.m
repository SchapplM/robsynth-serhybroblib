...
%   von `picker2Dm1DE2_invkin_eulangresidual`
%
% Ausgabe:
% Q
...
function [Q, QD, QDD, PHI] = picker2Dm1DE2_invkin_traj(X, XD, XDD, T, q0, s)
...
for k = 1:nt
  ...
  [q_k, Phi_k] = picker2Dm1DE2_invkin_eulangresidual(X(k,:)', qk0, s);

  %% Gelenk-Geschwindigkeit berechnen (Siehe [1]).
  % Geometrische Jacobi-Matrix in analytische Jacobi umrechnen
  Jg = picker2Dm1DE2_jacobig_sym_varpar(q_k, link_index, r_i_i_C, pkin);
  ...
  JgD = picker2Dm1DE2_jacobigD_sym_varpar(q_k, qD_k, link_index, r_i_i_C, pkin);
