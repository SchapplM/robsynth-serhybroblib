...
%   von `picker2Dm1OL_invkin_eulangresidual`
%
% Ausgabe:
% Q
...
function [Q, QD, QDD, PHI] = picker2Dm1OL_invkin_traj(X, XD, XDD, T, q0, s)

%% Coder Information
%#codegen
%$cgargs {coder.newtype('double',[inf,6]),coder.newtype('double',[inf,6]),
%$cgargs  coder.newtype('double',[inf,6]),coder.newtype('double',[inf,1]),
%$cgargs zeros(12,1), struct(
%$cgargs            'pkin', zeros(8,1),
%$cgargs          'sigmaJ', zeros(12,1),
%$cgargs             'NQJ', 0,
%$cgargs            'qlim', zeros(12,2),
...
for k = 1:nt
  ...
  [q_k, Phi_k] = picker2Dm1OL_invkin_eulangresidual(X(k,:)', qk0, s);

  %% Gelenk-Geschwindigkeit berechnen (Siehe [1]).
  % Geometrische Jacobi-Matrix in analytische Jacobi umrechnen
  Jg = picker2Dm1OL_jacobig_sym_varpar(q_k, link_index, r_i_i_C, pkin);
  ...
  JgD = picker2Dm1OL_jacobigD_sym_varpar(q_k, qD_k, link_index, r_i_i_C, pkin);
