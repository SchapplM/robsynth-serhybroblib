...
function T = picker2Dm1OL_energykin_fixb_slag_vp1(qJ, qJD, ...
  pkin, m, rSges, Icges)
%% Coder Information
%#codegen
%$cgargs {zeros(12,1),zeros(12,1),zeros(8,1),zeros(11,1),zeros(11,3),zeros(11,6)}
assert(isreal(qJ) && all(size(qJ) == [12 1]), ...
  'picker2Dm1OL_energykin_fixb_slag_vp1: qJ has to be [12x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [12 1]), ...
  'picker2Dm1OL_energykin_fixb_slag_vp1: qJD has to be [12x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [8 1]), ...
  'picker2Dm1OL_energykin_fixb_slag_vp1: pkin has to be [8x1] (double)');
assert(isreal(m) && all(size(m) == [11 1]), ...
  'picker2Dm1OL_energykin_fixb_slag_vp1: m has to be [11x1] (double)'); 
assert(isreal(rSges) && all(size(rSges) == [11,3]), ...
  'picker2Dm1OL_energykin_fixb_slag_vp1: rSges has to be [11x3] (double)');
assert(isreal(Icges) && all(size(Icges) == [11 6]), ...
  'picker2Dm1OL_energykin_fixb_slag_vp1: Icges has to be [11x6] (double)'); 

%% Symbolic Calculation
% From energy_kinetic_fixb_worldframe_par1_matlab.m
% OptimizationMode: 2
% StartTime: 2020-05-11 05:44:53
...
t1 = m(10) * (t282 ^ 2 + t283 ^ 2) / 0.2e1 + m(11) * (t280 ^ 2 + t281 ^ 2) / 0.2e1 + m(9) * (t290 ^ 2 + t292 ^ 2) / 0.2e1 + m(7) * (t288 ^ 2 + t289 ^ 2) / 0.2e1 + m(5) * (t284 ^ 2 + t286 ^ 2) / 0.2e1 + m(4) * (t285 ^ 2 + t287 ^ 2) / 0.2e1 + t306 ^ 2 * Icges(11,3) / 0.2e1 + t329 ^ 2 * Icges(9,3) / 0.2e1 + t307 ^ 2 * Icges(10,3) / 0.2e1 + t317 ^ 2 * Icges(7,3) / 0.2e1 + t318 ^ 2 * Icges(5,3) / 0.2e1 + t319 ^ 2 * Icges(4,3) / 0.2e1 + t330 ^ 2 * Icges(3,3) / 0.2e1 + m(3) * (t291 ^ 2 + t293 ^ 2) / 0.2e1 + (Icges(2,3) / 0.2e1 + m(2) * (t299 ^ 2 + t301 ^ 2) / 0.2e1) * qJD(1) ^ 2 + (m(6) * (t294 ^ 2 + t295 ^ 2) / 0.2e1 + Icges(6,3) / 0.2e1) * qJD(5) ^ 2 + (m(8) * (t298 ^ 2 + t300 ^ 2) / 0.2e1 + Icges(8,3) / 0.2e1) * qJD(7) ^ 2;
T = t1;
