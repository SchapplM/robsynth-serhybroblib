...
function jv = picker2Dm1DE1_kinconstr_expl_mdh_num_varpar(qJ, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(2,1),zeros(9,1)}
assert(isreal(qJ) && all(size(qJ) == [2 1]), ...
  'picker2Dm1DE1_kinconstr_expl_mdh_num_varpar: qJ has to be [2x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [9 1]), ...
  'picker2Dm1DE1_kinconstr_expl_mdh_num_varpar: pkin has to be [9x1] (double)');

%% Berechnung
% Kinematikparameter und Struktureigenschaften
[beta_mdh, b_mdh, alpha_mdh, a_mdh, theta_mdh, d_mdh, qoffset_mdh] = picker2Dm1DE1_pkin2mdhparam(pkin);
[~, sigma_mdh] = picker2Dm1DE1_structural_kinematic_parameters();

% Kinematik: Einzel-Gelenktransformationen
T = picker2Dm1DE1_joint_trafo_rotmat_mdh_sym_varpar(qJ, pkin);
