...
function Phi_a = fourbar1IC_kinconstr_impl_act_jacobian_mdh_sym_varpar(qJ, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(4,1),zeros(4,1)}
assert(isreal(qJ) && all(size(qJ) == [4 1]), ...
  'fourbar1IC_kinconstr_impl_act_jacobian_mdh_sym_varpar: qJ has to be [4x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [4 1]), ...
  'fourbar1IC_kinconstr_impl_act_jacobian_mdh_sym_varpar: pkin has to be [4x1] (double)');
...
t2 = qJ(1) + qJ(2);
t1 = [-pkin(3) * sin(t2) + sin(qJ(1)) * pkin(2); pkin(3) * cos(t2) - cos(qJ(1)) * pkin(2); -1;];
Phi_a = t1;
