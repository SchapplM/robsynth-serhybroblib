...
function WD = fourbar1TE_kinconstr_expl_jacobianD_mdh_sym_varpar(qJ, qJD, pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(1,1),zeros(1,1),zeros(4,1)}
assert(isreal(qJ) && all(size(qJ) == [1 1]), ...
  'fourbar1TE_kinconstr_expl_jacobianD_mdh_sym_varpar: qJ has to be [1x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [1 1]), ...
  'fourbar1TE_kinconstr_expl_jacobianD_mdh_sym_varpar: qJD has to be [1x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [4 1]), ...
  'fourbar1TE_kinconstr_expl_jacobianD_mdh_sym_varpar: pkin has to be [4x1] (double)');
...
t58 = t61 - t78;
t57 = t61 + t78;
t1 = [0; -pkin(1) * (t62 * t84 + (0.2e1 * t65 * t87 + (t58 * t67 + t80) * pkin(2)) * qJD(1)) * t82 + (-pkin(1) * t75 + t74 * t76) * (t58 * t86 + t62 * t72); pkin(2) * (t63 * t84 + (t65 * t76 + (t57 * t67 + t80) * pkin(1)) * qJD(1)) * t82 + (pkin(2) * t75 - 0.2e1 * t74 * t87) * (pkin(1) * t66 * t57 + t63 * t72); (t66 * t83 + t67 * t77) * pkin(1) * t90; 0;];
WD = t1;
