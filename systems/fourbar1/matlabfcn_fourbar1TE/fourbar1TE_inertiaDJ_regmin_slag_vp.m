...
function MMD_reg = fourbar1TE_inertiaDJ_regmin_slag_vp(qJ, qJD, ...
  pkin)
%% Coder Information
%#codegen
%$cgargs {zeros(1,1),zeros(1,1),zeros(4,1)}
assert(isreal(qJ) && all(size(qJ) == [1 1]), ...
  'fourbar1TE_inertiaDJ_regmin_slag_vp: qJ has to be [1x1] (double)');
assert(isreal(qJD) && all(size(qJD) == [1 1]), ...
  'fourbar1TE_inertiaDJ_regmin_slag_vp: qJD has to be [1x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [4 1]), ...
  'fourbar1TE_inertiaDJ_regmin_slag_vp: pkin has to be [4x1] (double)');

%% Symbolic Calculation
% From inertiaD_joint_joint_fixb_regressor_minpar_matlab.m
% OptimizationMode: 2
% StartTime: 2020-06-26 17:21:02
% EndTime: 2020-06-26 17:21:04
...
t11 = [0, 0, 0, t5 * t68 + (t5 * t69 + (t7 * (t3 + (0.2e1 * t82 + (t24 * t45 - t92) * pkin(1)) * qJD(1)) * t83 + t5 * t70) * t19) * t17, ((-t9 * t101 / 0.2e1 - t2 * t13 / 0.2e1) * t76 + (-((0.4e1 * t28 * t85 - 0.8e1 * t29 * t81) * t58 + 0.4e1 * t33 * t31 * t82 - 0.2e1 * t14 * t90 + t45 * t77) * t96 + t2 * t72) * t86) * t51, ((-t101 * t31 * t91 * t100 - t1 * t13) * t76 + ((0.3e1 * t15 * t85 - 0.12e2 * t16 * t81 + 0.12e2 * t44 * t74 + t73 * t99 - t80 * t90) * t83 + 0.8e1 * t1 * t75) * t86) * t51, t4 * t68 + (t4 * t69 + (t6 * (t3 + (-0.2e1 * t82 + (-t23 * t45 - t92) * pkin(1)) * qJD(1)) * t83 + t4 * t70) * t19) * t17, 0, 0;];
MMD_reg = t11;
